<template>
  <div class="media-info current-info" v-if="media">
    <div class="media-info-content current-info-content">
      <div class="media-info-content-block current-info-content-block">
        ...
        <p
          class="media-info-content-block-descr current-info-content-block-descr"
        >
          <span>{{
        ...
        <a
          :href="`http://www.youtube.com/watch/${trailer}`"
          target="_blank"
          v-if="trailer"
          class="btn-more"
          >смотреть трейлер</a
  ...
  <ul class="container media-content-list" v-if="media">
    <li>
      <h3>Бюджет</h3>
      <p v-if="media.budget">${{ getBudget }}</p>
      <p v-else>неизвесно...</p>
    </li>
    <li>
      <h3>Сбoры</h3>
      <p v-if="media.revenue">${{ getRevenue }}</p>
      <p v-else>неизвесно...</p>
    </li>
    <li>
      <h3>Статус</h3>
      <p>{{ media.status || "неизвесно..." }}</p>
    </li>
    <li>
      <h3>Исходное название</h3>
      <p>{{ media.original_title || "неизвесно..." }}</p>
    </li>
  </ul>
...
onMounted(async () => {
  ...
  trailer.value =
    props.type == "movie" ? trailerStore.movieTrailer : trailerStore.tvTrailer;
...
const getBudget = computed(() =>
  media.value.budget ? media.value.budget
        .toString()
        .split("")
        .reverse()
        .join("")
...
const getRevenue = computed(() =>
  media.value.revenue ? media.value.revenue
        .toString()
        .split("")
        .reverse()
        .join("")
